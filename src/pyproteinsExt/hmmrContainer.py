...
reType=re.compile('^\# hmm([\S]+) :: search ([\S]+) against a ([\S]+) database')
...
reDetail  = re.compile('Domain annotation for each sequence \(and alignments\):')
...
def parse(inputFile=None):
    ...
    if inputFile:
        ...
        with f:
            ...
            if not runReports:
                print("Could not parse up level:", inputFile)
                return Container()
            
            mainContainer = Container()
            for d in runReports:              
                mainContainer += Container(input=io.StringIO(d))
